...
public class VRWebView : MonoBehaviour
{
    ...
    {
        // Removed GUITexture check since it's obsolete and not needed for 3D objects.
        MeshFilter meshFilter = GetComponent<MeshFilter>();
        if (meshFilter == null)
        {
            enabled = false;
            Debug.LogError("VRWebView must be attached to a GameObject with a MeshFilter component.");
            return;
        }
        ...
        {
            enabled = false;
            Debug.LogError("VRWebView must be attached to a GameObject with a valid Mesh.");
            return;
        }

        // Store mesh vertices, UVs, and triangles for raycasting or other purposes
        m_RaycastMeshVertices = mesh.vertices;
        m_RaycastMeshUV = mesh.uv;
        m_RaycastMeshTriangles = mesh.triangles;

        // Check if we need to automatically add a BoxCollider
    ...
    protected void Start()
    {
        ...
        {
            Debug.LogError("[X] VRManager is missing from the scene!");
            enabled = false;
            return;
        }
        ...
        {
            // Get the vrCameras corresponding to Cameras
    ...
    m_PixelsHandle = GCHandle.Alloc(m_Pixels, GCHandleType.Pinned);
#else
        // Handle different pixel formats based on graphics API
        ...
        // Attach the texture to the appropriate component
        if (gameObject != null && gameObject.GetComponent<Renderer>() != null)
        {
            var renderer = gameObject.GetComponent<Renderer>();

            // Assign the material/shader to the object attached
            renderer.material = Resources.Load<Material>("VRWebViewMaterial");
            renderer.material.mainTexture = this.m_Texture;
        }
        else
        {
            Debug.LogError("VRWebView must be attached to a GameObject with a Renderer component!");
            enabled = false;
            return;
        }
        ...
        if (!MiddleVR.VRClusterMgr.IsClient())
        {
            ...
            else
            {
                ...
                {
                    m_WebView.AddDirectoryProvider(
                        directoryProvider.m_URL,
                        Application.dataPath + System.IO.Path.DirectorySeparatorChar + directoryProvider.m_DirectoryPath,
                        order,
                        "VRWebViewProvider" + order
                    );
                    order++;
                }
                ...
                {
                    m_WebView.AddArchiveProvider(
                        archiveProvider.m_URL,
                        Application.dataPath + System.IO.Path.DirectorySeparatorChar + archiveProvider.m_ArchivePath,
                        archiveProvider.m_Password,
                        order,
                        "VRWebViewProvider" + order
                    );
                    order++;
    ...
    protected void Update()
    {
        ...
        if (!MiddleVR.VRClusterMgr.IsClient())
        {
            if (!m_IgnorePhysicalMouseInput)
            {
                ...
                // Handling 2D UI interactions (UI.Image instead of GUITexture)
                Image uiImage = gameObject.GetComponent<Image>();

                if (m_MouseInteractionsIn2D && uiImage != null)
                {
                    // Get the RectTransform of the UI Image for 2D mouse input
                    ...
                    // Convert mouse position to normalized coordinates (0 to 1) within the UI Image
                    Rect rect = rectTransform.rect;
                    if (rect.Contains(localMousePosition))
                    {
                        float x = (localMousePosition.x - rect.x) / rect.width;
                        float y = (localMousePosition.y - rect.y) / rect.height;
