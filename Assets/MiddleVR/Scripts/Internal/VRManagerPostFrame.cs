...
using UnityEngine.UI; // Add this for the new UI system
...
public class VRManagerPostFrame : MonoBehaviour
{
    ...
    // Reference to the Text component to replace GUIText
    public Text uiText; // Assign this from the Inspector
    ...
    // This coroutine will wait for the end of each frame and then
    ...
    private IEnumerator PostFrameUpdate()
    {
        while (ContinuePostFrameUpdate)
        {
            ...
            if (mgr != null && MiddleVR.VRKernel == null)
            {
                ...
                // Replacing GUIText with UI.Text
                if (uiText != null)
                {
                    uiText.text = "[ ] Check the console window to check if you have an error mentioning 'DLLNotFoundException: MiddleVR_CSharp', please restart Unity. If this does not fix the problem, please make sure MiddleVR is in the PATH environment variable.";
                }
            }
