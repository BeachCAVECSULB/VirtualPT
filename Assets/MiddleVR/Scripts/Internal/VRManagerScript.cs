...
public class VRManagerScript : MonoBehaviour
{
    ...
    // doesn't have a lot of stuff from drone project
    private void _EnableFPSDisplay(bool iEnable)
    {
        m_ShowFPS = iEnable;

        // Get the Text component (UI.Text) instead of GUIText
        Text fpsText = this.GetComponent<Text>();

        if (fpsText != null)
        {
            // Enable or disable the FPS display based on iEnable
            fpsText.enabled = m_ShowFPS;
        }
        else
        {
    ...
    protected void Update()
    {
        ...
        if (m_isInit)
        {
            ...
            {
                // Set the random seed in the kernel for dispatching only during start-up.
                // With clustering, a client will be set by a call to kernel.Update().
                if (!m_ClusterMgr.IsCluster() || (m_ClusterMgr.IsCluster() && m_ClusterMgr.IsServer()))
                {
                    uint seed = (uint)Random.Range(1, int.MaxValue); // Use Random.Range to generate a seed
            ...
            if (m_Kernel.GetFrame() == 0)
            {
                ...
                if (m_ClusterMgr.IsCluster() && m_ClusterMgr.IsClient())
                {
                    ...
                    Random.InitState(seed); // Use Random.InitState instead of Random.seed
                }
            }
            ...
            if (ShowFPS)
            {
                // Use Text component instead of GUIText for showing FPS
                Text fpsText = this.GetComponent<Text>();
                if (fpsText != null)
                {
                    fpsText.text = m_Kernel.GetFPS().ToString("f2");
                }
            }
            ...
            if (m_displayLog)
            {
                string txt = m_Kernel.GetLogString(true);
                print(txt);
                m_GUI.text = txt; // Assuming m_GUI is a UI Text component
            }
        ...
        {
            // Debug or warning if needed
        }

        // If QualityLevel changed, reset the Unity Manager
